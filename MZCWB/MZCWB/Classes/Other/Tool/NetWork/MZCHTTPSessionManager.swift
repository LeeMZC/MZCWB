...
class MZCAlamofire : NSObject{
    ...
    //MARK:- 微博发送数据
    func sendMessage(text aText : String , finished : (error : NSError?)->()) {
        
        let urlString = baseURL + "2/statuses/home_timeline.json"
        let parameters = ["access_token": (accountTokenMode?.access_token)!, "status": aText]
...
//MARK:- private
extension MZCAlamofire {
    //MARK:- 将数组写入桌面
    private func mzc_writeToFile(arr: [[String:AnyObject]]){
